...
class MafengwoYoujiSpider(CrawlSpider):
    ...
    def parse(self, response):
        ...
        max_youji_page=int(max_youji_page)
    ...
    def parse_youji(self,response):
        ...
        if url_page:
            max_page_m=sel.xpath('//a[@class="ti"]/@data-value').extract()
            if max_page_m:
                max_page=max_page_m[-1]
            else:
                max_page_m=sel.xpath('//a[@class="ti"]/text()').extract()
                if max_page_m:
                    max_page=max_page_m[-1]
                else:
                    max_page=1
    ...
    def parse_reply(self,response):
        ...
        if page<max_page:
            ...
            return Request(url=url, callback=self.parse_reply, meta={'item': item, 'page': page ,'max_page':max_page ,'url':url})
        else:
            items.append(item)
            return items
