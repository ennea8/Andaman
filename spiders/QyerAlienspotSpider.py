...
class QyerAlienSpotSpider(CrawlSpider):
    ...
    def parse_countrysights(self, response):
        ...
        tmp = sel.xpath('//div[@id="place_memu_fix"]/div/div[@class="pla_topbtns"]/a[@class="ui_button yelp"]/@onclick').extract()
        if tmp:
        ...
        if tmp:
            ...
            if pagenum>1:
                for page in range(pagenum):
                    country_info = copy.deepcopy(country)
                    body = 'action=ajaxpoi&page=%d&pagesize=16&id=%d&typename=country&cateid=32&orderby=0&tagid=0'%(page+2,country_id)
    ...
    def parse_list(self,response):
        ...
        sel = HtmlXPathSelector(text=data['data']['html'])
